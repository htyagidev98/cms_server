...
exports.imageCardAdd = async (req, res, images) => {
    try {
        ...
        if (validation.fails()) {
            ...
            let networkData = await ImageCard.findOne({ title: title }).lean();
            if (!networkData) {
...
exports.imageCardUpdate = async (req, res, images) => {
    try {
        ...
        if (validation.fails()) {
            ...
            const { title, paragraph } = req.body;
            const { _id } = req.query;
            let networkData = await ImageCard.findById(_id).lean();
            if (!networkData) {
...
exports.imageCardDelete = async (req, res) => {
    try {
        ...
        let networkData = await ImageCard.findById(_id).lean();
        if (networkData) {
            await ImageCard.findByIdAndDelete({ _id: networkData._id }, networkData, { new: true });
