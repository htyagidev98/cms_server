const Azero = require('../../models/azero')
bodyParser = require("body-parser")
Validator = require("validatorjs")



exports.azeroUtilityAdd = async (req, res) => {
    try {
        ...
        if (validation.fails()) {
            ...
            let azeroData = await Azero.findOne({ title: title }).lean();
            if (!azeroData) {

                let data = await Azero.create({
                    title: title,
                    paragraph: paragraph,
                });
...
exports.azeroUtilityUpdate = async (req, res,) => {
    try {
        const rules = { title: "required", paragraph: "required" };
        ...
        if (validation.fails()) {
            ...
            const { title, paragraph } = req.body;
            const { _id } = req.query;
            let azeroData = await Azero.findById(_id).lean();
            if (azeroData) {
                let updatedData = {
                    title: title,
                    paragraph: paragraph,
