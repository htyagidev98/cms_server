...
exports.newsAdd = async (req, res) => {
    try {
        ...
        if (validation.fails()) {
            ...
            let newsData = await News.findOne({ title: title }).lean();
            if (!newsData) {
...
exports.newsGet = async (req, res) => {
    try {
        const contentlist = await News.findOne().lean();
        if (contentlist) {
            const contentObj = {
                _id: contentlist._id,
...
exports.newsUpdate = async (req, res) => {
    ...
    if (validation.fails()) {
        ...
        const { title, paragraph } = req.body;
        const { _id } = req.query;
        let newsData = await News.findById(_id).lean();
        if (!newsData) {
            ...
            const data = await News.findByIdAndUpdate({ _id: newsData._id }, updatedData, { new: true });
