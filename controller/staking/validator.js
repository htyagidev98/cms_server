...
exports.validatorAdd = async (req, res) => {
    try {
        ...
        if (validation.fails()) {
            ...
            let StakingValidatorData = await StakingValidator.findOne({ title: title }).lean();
            if (!StakingValidatorData) {
...
exports.validatorGet = async (req, res) => {
    try {
        const contentlist = await StakingValidator.findOne().sort({ createdAt: 1 });
        if (contentlist) {
            const contentObj = {
                _id: contentlist._id,
...
exports.validatorUpdate = async (req, res) => {
    try {
        ...
        if (validation.fails()) {
            ...
            const { title, paragraph } = req.body;
            const { _id } = req.query;
            let StakingValidatorData = await StakingValidator.findById(_id).lean();
            if (!StakingValidatorData) {
                ...
                const data = await StakingValidator.findByIdAndUpdate({ _id: _id }, updatedData, { new: true });
