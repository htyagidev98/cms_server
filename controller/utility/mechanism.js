...
exports.mechanismAdd = async (req, res) => {
    try {
        ...
        if (validation.fails()) {
            ...
            let mechanismData = await Mechanism.findOne({ title: title }).lean();
            if (!mechanismData) {

                let data = await Mechanism.create({
                    title: title,
                    paragraph: paragraph,
                });
...
exports.mechanismUpdate = async (req, res,) => {
    try {
        const rules = { title: "required", paragraph: "required" };
