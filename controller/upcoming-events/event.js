...
exports.eventAdd = async (req, res) => {
    try {
        ...
        if (validation.fails()) {
            ...
            let eventData = await Event.findOne({ title: title }).lean();
            if (!eventData) {
...
exports.eventGet = async (req, res) => {
    try {
        const contentlist = await Event.findOne().lean();
        if (contentlist) {
            const contentObj = {
                _id: contentlist._id,
...
exports.eventUpdate = async (req, res) => {
    ...
    if (validation.fails()) {
        ...
        const { title, paragraph } = req.body;
        const { _id } = req.query;
        let eventData = await Event.findById(_id).lean();
        if (!eventData) {
            ...
            const data = await Event.findByIdAndUpdate({ _id: eventData._id }, updatedData, { new: true });
